...
void solve(const int NUMBER) {
	...
	const std::string black_suffix = BLACK_ONLY_FLAG ? "_black" : "";
...

	{
		...
		writing_file.open("dimacs_cnf_" + std::to_string(NUMBER) + black_suffix + "_cpp.txt", std::ios::out);
	...
	}
	{
		std::ofstream writing_file;
		writing_file.rdbuf()->pubsetbuf(buf, BUFSIZE);
		writing_file.open("dimacs_literal_2_token_" + std::to_string(NUMBER) + black_suffix + ".csv", std::ios::out);
...
int main(int argc, char *argv[]) {
	...
	if (argc == 2) {
		if (std::string(argv[1]) == "33") {
			BLACK_ONLY_FLAG = false;
			solve(33);
		}
		else if (std::string(argv[1]) == "34") {
			BLACK_ONLY_FLAG = false;
			solve(34);
		}
		else if (std::string(argv[1]) == "33black") {
			BLACK_ONLY_FLAG = true;
			solve(33);
		}
	}
	return 0;
